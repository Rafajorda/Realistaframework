...
@session_start();
// if (isset($_SESSION["tiempoahora"])) {  
//     $_SESSION["tiempoahora"] = time(); //Devuelve la fecha actual
// } 
...
switch ($_GET['op']) {
        ...
        try {
            ...
            if ($rdo == "error_user") {
                ...
                if (password_verify($_POST['passwd_log'], $rdo['password'])) {
                     $accesstoken= create_accesstoken($rdo["username"],$rdo["id_user"]);
                     $refreshtoken= create_refreshtoken($rdo["username"],$rdo["id_user"]);
    ...
    case 'controluser':
        $token_dec = decode_token($_POST['accesstoken']);
    ...
    case 'expires':
        $refreshtoken_dec = decode_token($_POST['refreshtoken']);
        $accesstoken_dec = decode_token($_POST['accesstoken']);
        ...
        if($accesstoken_dec['username']==$refreshtoken_dec['username']){
            $new_token = create_accesstoken($refreshtoken_dec['username'],$refreshtoken_dec['id']);
